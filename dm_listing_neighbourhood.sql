...
SELECT listing_neighbourhood,
       ...
       CAST(100*COUNT(DISTINCT host_id) FILTER (WHERE host_is_superhost='t')::FLOAT/NULLIF(COUNT(DISTINCT host_id),0)::FLOAT AS DECIMAL(4,1)) AS superhost_rate,
       ...
       ) OVER(PARTITION BY listing_neighbourhood ORDER BY year,month))::FLOAT/LAG(COUNT(CASE WHEN HAS_AVAILABILITY='t' THEN 1 ELSE NULL END)
       ) OVER(PARTITION BY listing_neighbourhood ORDER BY year,month)::FLOAT AS DECIMAL(32,1))AS pct_change_active_listings,
       ...
       ) OVER(PARTITION BY listing_neighbourhood ORDER BY year,month))::FLOAT/NULLIF(LAG(COUNT(CASE WHEN HAS_AVAILABILITY='f' THEN 1 ELSE NULL END)
       ) OVER(PARTITION BY listing_neighbourhood ORDER BY year,month),0)::FLOAT AS DECIMAL(32,1))AS pct_change_inactive_listings,
...
FROM source
GROUP BY month, year, listing_neighbourhood
